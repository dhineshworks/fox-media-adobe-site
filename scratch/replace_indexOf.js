...
const p1 = html.indexOf('function BulkSearchPage');
const p2 = html.indexOf('// ─── GENERATOR PAGE', p1);

if (p1 !== -1 && p2 !== -1) {
  const before = html.substring(0, p1);
  const after = html.substring(p2 + '// ─── GENERATOR PAGE ────────────────────────────────────────────────────────────'.length);
  html = before + replacement + after;
  fs.writeFileSync('../index.html', html, 'utf8');
  console.log("Replaced successfully.");
} else {
  console.log("Indexes not found", p1, p2);
}
