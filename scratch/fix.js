      ...
      return (
        <div className="page-content" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 20px', background: '#0f172a', minHeight: 'calc(100vh - 64px)' }}>
          <div style={{ maxWidth: '800px', width: '100%', margin: '0 auto' }}>
            ...
            {/* Title & Subtitle */}
            <h1 style={{ textAlign: 'center', fontSize: '32px', color: '#fff', marginBottom: '16px', fontWeight: 'bold' }}>Bulk Status Checker</h1>
            ...
            <div style={{ background: '#1e293b', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)' }}>
              <textarea 
                style={{ width: '100%', background: '#0f172a', border: '1px solid rgba(255, 255, 255, 0.05)', color: '#fff', padding: '16px', minHeight: '160px', borderRadius: '8px', fontSize: '14px', fontFamily: 'monospace', resize: 'vertical', marginBottom: '24px' }}
...
// Fallback for CRLF in BulkSearchPage block
if (html.includes('iframe')) {
  // It probably didn't replace because of CRLF or whitespace
  html = html.replace(/function BulkSearchPage\(\{ toast \}\) \{[\s\S]*?\}\s*\/\//m, `function BulkSearchPage({ toast }) {
      const [inputEmails, setInputEmails] = useState("");
      const [loading, setLoading] = useState(false);
      const [results, setResults] = useState(null);
      const [progress, setProgress] = useState(0);
